...
namespace Pt5Reader
{
    ...
    class Program
    {
        static void Main(string[] args)
        {
...
#endif
            FileStream pt5Stream = File.Open(fileName,
                                              FileMode.Open,
                                              FileAccess.Read,
                                              FileShare.ReadWrite);
            BinaryReader pt5Reader = new BinaryReader(pt5Stream);

            // Read the file header
            PT5.Pt5Header header = new PT5.Pt5Header();
            PT5.ReadHeader(pt5Reader, ref header);
#if DEBUG
            ...
            if (fileName.EndsWith(".pt3") || fileName.EndsWith(".pt4"))
            {
                Console.WriteLine("Exiting, PT3 or PT4 file found.  Only PT5 format files are fully supported.");
                //cleanup
                pt5Reader.Close();
#if DEBUG
            ...
            PT5.StatusPacket statusPacket = new PT5.StatusPacket();
            PT5.ReadStatusPacket(pt5Reader, ref statusPacket);


            // Determine the number of samples in the file
            long sampleCount = PT5.SampleCount(pt5Reader,
            ...
            // routine)
            pt5Reader.BaseStream.Position = PT5.sampleOffset;

            long missingCount = 0;

            // Process the samples sequentially, beginning to end
            PT5.Sample sample = new PT5.Sample();
            for (long sampleIndex = 0;
                 sampleIndex < sampleCount;
                 sampleIndex++)
            {
                // Read the next sample
                PT5.GetSample(sampleIndex, header.captureDataMask,
